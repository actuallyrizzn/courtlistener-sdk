"""
Tests for Investments API.
"""

import pytest
from unittest.mock import Mock, patch
from courtlistener.api.investments import InvestmentsAPI


class TestInvestmentsAPI:
    """Test cases for Investments API."""
    
    def setup_method(self):
        """Set up test fixtures."""
        self.mock_client = Mock()
        self.api = InvestmentsAPI(self.mock_client)
    
    def test_list_investments(self):
        """Test listing investments."""
        mock_response = {
            "count": 1,
            "next": None,
            "previous": None,
            "results": [
                {
                    "id": 1,
                    "financial_disclosure": "https://api.courtlistener.com/api/rest/v4/financial-disclosures/123/",
                    "judge": "https://api.courtlistener.com/api/rest/v4/judges/456/",
                    "description": "Stock investment",
                    "value_min": 1000.0,
                    "value_max": 5000.0
                }
            ]
        }
        self.mock_client.get.return_value = mock_response
        
        result = self.api.list(financial_disclosure=123)
        
        assert result == mock_response
        self.mock_client.get.assert_called_once_with(
            "investments/",
            params={"financial_disclosure": 123}
        )
    
    def test_list_investments_with_judge_filter(self):
        """Test listing investments with judge filter."""
        mock_response = {"count": 0, "results": []}
        self.mock_client.get.return_value = mock_response
        
        result = self.api.list(judge=456)
        
        expected_params = {"judge": 456}
        self.mock_client.get.assert_called_once_with("investments/", params=expected_params)
    
    def test_get_investment(self):
        """Test getting a specific investment."""
        mock_response = {
            "id": 1,
            "financial_disclosure": "https://api.courtlistener.com/api/rest/v4/financial-disclosures/123/",
            "description": "Stock investment"
        }
        self.mock_client.get.return_value = mock_response
        
        result = self.api.get(1)
        
        assert result == mock_response
        self.mock_client.get.assert_called_once_with("investments/1/")
    
    def test_paginate_investments(self):
        """Test paginating investments."""
        mock_iterator = Mock()
        self.mock_client.paginate.return_value = mock_iterator
        
        result = self.api.paginate(financial_disclosure=123)
        
        assert result == mock_iterator
        self.mock_client.paginate.assert_called_once_with(
            "investments/",
            params={"financial_disclosure": 123}
        )
    
    def test_list_with_additional_params(self):
        """Test listing investments with additional parameters."""
        mock_response = {"count": 0, "results": []}
        self.mock_client.get.return_value = mock_response
        
        result = self.api.list(financial_disclosure=123, page=2, page_size=50)
        
        expected_params = {
            "financial_disclosure": 123,
            "page": 2,
            "page_size": 50
        }
        self.mock_client.get.assert_called_once_with("investments/", params=expected_params)
