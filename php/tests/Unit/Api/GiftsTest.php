...
class GiftsTest extends TestCase
{
    ...
    public function testList()
    {
        $expectedResponse = [
            'count' => 100,
            'next' => 'http://api.example.com/gifts/?page=2',
            'previous' => null,
            'results' => [
    ...
    public function testSearch()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'gifts/search/', ['query' => ['q' => 'test']])
            ->willReturn($expectedResponse);
    ...
    public function testPatch()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('PATCH', 'gifts/123/', ['json' => $data])
            ->willReturn($expectedResponse);
    ...
    public function testListWithPagination()
    {
        $expectedResponse = [
            'count' => 200,
            'next' => 'http://api.example.com/gifts/?page=3',
            'previous' => 'http://api.example.com/gifts/?page=1',
            'results' => []
        ];
    ...
    public function testSearchWithComplexFilters()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'gifts/search/', ['query' => $searchParams])
            ->willReturn($expectedResponse);
    ...
    public function testGetWithParams()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'gifts/789/', ['query' => $params])
            ->willReturn($expectedResponse);
    ...
    public function testEndpointName()
    {
        ...
        $this->assertEquals('Gifts', $reflection->getShortName());
    }
