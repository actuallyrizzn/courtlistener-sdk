...
{
    private Clusters $endpoint;
    private MockObject $client;

    protected function setUp(): void
    {
        $this->client = $this->createMock(CourtListenerClient::class);
        $this->endpoint = new Clusters($this->client);
    }
    ...
    public function testList()
    {
        $expectedResponse = [
            'count' => 100,
            'next' => 'http://api.example.com/clusters/?page=2',
            'previous' => null,
            'results' => [
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'clusters/', ['query' => ['page' => 1]])
    ...
    public function testSearch()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'clusters/search/', ['query' => ['q' => 'test']])
            ->willReturn($expectedResponse);
    ...
    public function testUpdate()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('PUT', 'clusters/123/', ['json' => $data])
            ->willReturn($expectedResponse);
    ...
    public function testPatch()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('PATCH', 'clusters/123/', ['json' => $data])
            ->willReturn($expectedResponse);
    ...
    public function testListWithPagination()
    {
        $expectedResponse = [
            'count' => 200,
            'next' => 'http://api.example.com/clusters/?page=3',
            'previous' => 'http://api.example.com/clusters/?page=1',
            'results' => []
        ];
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'clusters/', ['query' => $params])
            ->willReturn($expectedResponse);
    ...
    public function testListWithFilters()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'clusters/', ['query' => $filters])
            ->willReturn($expectedResponse);
    ...
    public function testSearchWithComplexFilters()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'clusters/search/', ['query' => $searchParams])
            ->willReturn($expectedResponse);
    ...
    public function testGetWithParams()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'clusters/789/', ['query' => $params])
            ->willReturn($expectedResponse);
    ...
    public function testEndpointName()
    {
        ...
        $this->assertEquals('Clusters', $reflection->getShortName());
    }
