...
class CourtsTest extends TestCase
{
    ...
    public function testList()
    {
        $expectedResponse = [
            'count' => 100,
            'next' => 'http://api.example.com/courts/?page=2',
            'previous' => null,
            'results' => [
    ...
    public function testSearch()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'courts/search/', ['query' => ['q' => 'test']])
            ->willReturn($expectedResponse);
    ...
    public function testPatch()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('PATCH', 'courts/123/', ['json' => $data])
            ->willReturn($expectedResponse);
    ...
    public function testListWithPagination()
    {
        $expectedResponse = [
            'count' => 200,
            'next' => 'http://api.example.com/courts/?page=3',
            'previous' => 'http://api.example.com/courts/?page=1',
            'results' => []
        ];
    ...
    public function testSearchWithComplexFilters()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'courts/search/', ['query' => $searchParams])
            ->willReturn($expectedResponse);
    ...
    public function testGetWithParams()
    {
        ...
        $this->client->expects($this->once())
            ->method('makeRequest')
            ->with('GET', 'courts/789/', ['query' => $params])
            ->willReturn($expectedResponse);
    ...
    public function testEndpointName()
    {
        ...
        $this->assertEquals('Courts', $reflection->getShortName());
    }
