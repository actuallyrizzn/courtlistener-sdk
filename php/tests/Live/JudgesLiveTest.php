...
/**
 * Live API tests for Judges endpoint
 * 
 * @group live
 * @group network
 * @group judges
 */
class JudgesLiveTest extends TestCase
{
    ...
    public function testListJudges()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->judges->list(Pagination::getParams(1, 5));
    ...
    public function testSearchJudges()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->judges->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetJudgesById()
    {
        ...
        // Validate response$results = $this->client->judges->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->judges->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Judges available for testing');
        }
    }

    public function testJudgesWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->judges->list(Pagination::getParams(1, 2));
        $page2 = $this->client->judges->list(Pagination::getParams(2, 2));
    ...
    public function testJudgesWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->judges->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testJudgesErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->judges->get(999999999);
    ...
    public function testJudgesSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->judges->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testJudgesResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Judges available for structure testing');
        }
    }

    public function testJudgesRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->judges->list(Pagination::getParams(1, 1));
                $requests++;
