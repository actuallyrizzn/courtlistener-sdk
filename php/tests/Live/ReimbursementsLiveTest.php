...
/**
 * Live API tests for Reimbursements endpoint
 * 
 * @group live
 * @group network
 * @group reimbursements
 */
class ReimbursementsLiveTest extends TestCase
{
    ...
    public function testListReimbursements()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->reimbursements->list(Pagination::getParams(1, 5));
    ...
    public function testSearchReimbursements()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->reimbursements->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetReimbursementsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->reimbursements->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->reimbursements->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Reimbursements available for testing');
        }
    }

    public function testReimbursementsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->reimbursements->list(Pagination::getParams(1, 2));
        $page2 = $this->client->reimbursements->list(Pagination::getParams(2, 2));
    ...
    public function testReimbursementsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->reimbursements->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testReimbursementsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->reimbursements->get(999999999);
    ...
    public function testReimbursementsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->reimbursements->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testReimbursementsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Reimbursements available for structure testing');
        }
    }

    public function testReimbursementsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->reimbursements->list(Pagination::getParams(1, 1));
                $requests++;
