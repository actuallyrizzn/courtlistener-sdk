...
/**
 * Live API tests for FJC Integrated Database endpoint
 * 
 * @group live
 * @group network
 * @group fjc-integrated-database
 */
class FjcIntegratedDatabaseLiveTest extends TestCase
{
    ...
    public function testListFjcIntegratedDatabase()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->fjcIntegratedDatabase->list(Pagination::getParams(1, 5));
    ...
    public function testSearchFjcIntegratedDatabase()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->fjcIntegratedDatabase->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetFjcIntegratedDatabaseById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->fjcIntegratedDatabase->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->fjcIntegratedDatabase->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No FJC Integrated Database available for testing');
        }
    }

    public function testFjcIntegratedDatabaseWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->fjcIntegratedDatabase->list(Pagination::getParams(1, 2));
        $page2 = $this->client->fjcIntegratedDatabase->list(Pagination::getParams(2, 2));
    ...
    public function testFjcIntegratedDatabaseWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->fjcIntegratedDatabase->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testFjcIntegratedDatabaseErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->fjcIntegratedDatabase->get(999999999);
    ...
    public function testFjcIntegratedDatabaseSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->fjcIntegratedDatabase->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testFjcIntegratedDatabaseResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No FJC Integrated Database available for structure testing');
        }
    }

    public function testFjcIntegratedDatabaseRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->fjcIntegratedDatabase->list(Pagination::getParams(1, 1));
                $requests++;
