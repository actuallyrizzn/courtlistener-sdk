...
/**
 * Live API tests for Docket Entries endpoint
 * 
 * @group live
 * @group network
 * @group docket-entries
 */
class DocketEntriesLiveTest extends TestCase
{
    ...
    public function testListDocketEntries()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->docketEntries->list(Pagination::getParams(1, 5));
    ...
    public function testSearchDocketEntries()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->docketEntries->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetDocketEntriesById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->docketEntries->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->docketEntries->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Docket Entries available for testing');
        }
    }

    public function testDocketEntriesWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->docketEntries->list(Pagination::getParams(1, 2));
        $page2 = $this->client->docketEntries->list(Pagination::getParams(2, 2));
    ...
    public function testDocketEntriesWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->docketEntries->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testDocketEntriesErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->docketEntries->get(999999999);
    ...
    public function testDocketEntriesSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->docketEntries->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testDocketEntriesResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Docket Entries available for structure testing');
        }
    }

    public function testDocketEntriesRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->docketEntries->list(Pagination::getParams(1, 1));
                $requests++;
