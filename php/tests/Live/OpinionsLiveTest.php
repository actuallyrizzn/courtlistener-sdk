...
/**
 * Live API tests for Opinions endpoint
 * 
 * @group live
 * @group network
 * @group opinions
 */
class OpinionsLiveTest extends TestCase
{
    ...
    public function testListOpinions()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->opinions->list(Pagination::getParams(1, 5));
    ...
    public function testSearchOpinions()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->opinions->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetOpinionsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->opinions->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->opinions->get($itemId);
    ...
    public function testOpinionsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->opinions->list(Pagination::getParams(1, 2));
        $page2 = $this->client->opinions->list(Pagination::getParams(2, 2));
    ...
    public function testOpinionsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->opinions->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testOpinionsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->opinions->get(999999999);
    ...
    public function testOpinionsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->opinions->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testOpinionsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Opinions available for structure testing');
        }
    }

    public function testOpinionsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->opinions->list(Pagination::getParams(1, 1));
                $requests++;
