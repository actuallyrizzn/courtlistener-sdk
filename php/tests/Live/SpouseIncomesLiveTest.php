...
/**
 * Live API tests for Spouse Incomes endpoint
 * 
 * @group live
 * @group network
 * @group spouse-incomes
 */
class SpouseIncomesLiveTest extends TestCase
{
    ...
    public function testListSpouseIncomes()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->spouseIncomes->list(Pagination::getParams(1, 5));
    ...
    public function testSearchSpouseIncomes()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->spouseIncomes->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetSpouseIncomesById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->spouseIncomes->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->spouseIncomes->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Spouse Incomes available for testing');
        }
    }

    public function testSpouseIncomesWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->spouseIncomes->list(Pagination::getParams(1, 2));
        $page2 = $this->client->spouseIncomes->list(Pagination::getParams(2, 2));
    ...
    public function testSpouseIncomesWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->spouseIncomes->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testSpouseIncomesErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->spouseIncomes->get(999999999);
    ...
    public function testSpouseIncomesSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->spouseIncomes->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testSpouseIncomesResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Spouse Incomes available for structure testing');
        }
    }

    public function testSpouseIncomesRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->spouseIncomes->list(Pagination::getParams(1, 1));
                $requests++;
