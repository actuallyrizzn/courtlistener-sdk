...
/**
 * Live API tests for Financial Disclosures endpoint
 * 
 * @group live
 * @group network
 * @group financial-disclosures
 */
class FinancialDisclosuresLiveTest extends TestCase
{
    ...
    public function testListFinancialDisclosures()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->financialDisclosures->list(Pagination::getParams(1, 5));
    ...
    public function testSearchFinancialDisclosures()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->financialDisclosures->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetFinancialDisclosuresById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->financialDisclosures->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->financialDisclosures->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Financial Disclosures available for testing');
        }
    }

    public function testFinancialDisclosuresWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->financialDisclosures->list(Pagination::getParams(1, 2));
        $page2 = $this->client->financialDisclosures->list(Pagination::getParams(2, 2));
    ...
    public function testFinancialDisclosuresWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->financialDisclosures->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testFinancialDisclosuresErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->financialDisclosures->get(999999999);
    ...
    public function testFinancialDisclosuresSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->financialDisclosures->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testFinancialDisclosuresResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Financial Disclosures available for structure testing');
        }
    }

    public function testFinancialDisclosuresRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->financialDisclosures->list(Pagination::getParams(1, 1));
                $requests++;
