...
/**
 * Live API tests for Sources endpoint
 * 
 * @group live
 * @group network
 * @group sources
...
class SourcesLiveTest extends TestCase
{
    ...
    public function testListSources()
    {
        ...
        // Validate response$results = $this->client->sources->list(Pagination::getParams(1, 5));
    ...
    public function testSearchSources()
    {
        ...
        $searchResults = $this->client->sources->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetSourcesById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->sources->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->sources->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Sources available for testing');
        }
    }

    public function testSourcesWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->sources->list(Pagination::getParams(1, 2));
        $page2 = $this->client->sources->list(Pagination::getParams(2, 2));
    ...
    public function testSourcesWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->sources->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testSourcesErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->sources->get(999999999);
    ...
    public function testSourcesSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->sources->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testSourcesResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Sources available for structure testing');
        }
    }

    public function testSourcesRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->sources->list(Pagination::getParams(1, 1));
                $requests++;
