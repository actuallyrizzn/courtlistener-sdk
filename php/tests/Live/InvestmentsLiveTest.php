...
/**
 * Live API tests for Investments endpoint
 * 
 * @group live
 * @group network
 * @group investments
 */
class InvestmentsLiveTest extends TestCase
{
    ...
    public function testListInvestments()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->investments->list(Pagination::getParams(1, 5));
    ...
    public function testSearchInvestments()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->investments->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetInvestmentsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->investments->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->investments->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Investments available for testing');
        }
    }

    public function testInvestmentsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->investments->list(Pagination::getParams(1, 2));
        $page2 = $this->client->investments->list(Pagination::getParams(2, 2));
    ...
    public function testInvestmentsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->investments->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testInvestmentsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->investments->get(999999999);
    ...
    public function testInvestmentsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->investments->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testInvestmentsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Investments available for structure testing');
        }
    }

    public function testInvestmentsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->investments->list(Pagination::getParams(1, 1));
                $requests++;
