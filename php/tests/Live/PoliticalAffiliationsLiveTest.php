...
/**
 * Live API tests for Political Affiliations endpoint
 * 
 * @group live
 * @group network
 * @group political-affiliations
 */
class PoliticalAffiliationsLiveTest extends TestCase
{
    ...
    public function testListPoliticalAffiliations()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->politicalAffiliations->list(Pagination::getParams(1, 5));
    ...
    public function testSearchPoliticalAffiliations()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->politicalAffiliations->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetPoliticalAffiliationsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->politicalAffiliations->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->politicalAffiliations->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Political Affiliations available for testing');
        }
    }

    public function testPoliticalAffiliationsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->politicalAffiliations->list(Pagination::getParams(1, 2));
        $page2 = $this->client->politicalAffiliations->list(Pagination::getParams(2, 2));
    ...
    public function testPoliticalAffiliationsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->politicalAffiliations->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testPoliticalAffiliationsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->politicalAffiliations->get(999999999);
    ...
    public function testPoliticalAffiliationsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->politicalAffiliations->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testPoliticalAffiliationsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Political Affiliations available for structure testing');
        }
    }

    public function testPoliticalAffiliationsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->politicalAffiliations->list(Pagination::getParams(1, 1));
                $requests++;
