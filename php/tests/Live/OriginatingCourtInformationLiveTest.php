...
/**
 * Live API tests for Originating Court Information endpoint
 * 
 * @group live
 * @group network
 * @group originating-court-information
 */
class OriginatingCourtInformationLiveTest extends TestCase
{
    ...
    public function testListOriginatingCourtInformation()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->originatingCourtInformation->list(Pagination::getParams(1, 5));
    ...
    public function testSearchOriginatingCourtInformation()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->originatingCourtInformation->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetOriginatingCourtInformationById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->originatingCourtInformation->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->originatingCourtInformation->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Originating Court Information available for testing');
        }
    }

    public function testOriginatingCourtInformationWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->originatingCourtInformation->list(Pagination::getParams(1, 2));
        $page2 = $this->client->originatingCourtInformation->list(Pagination::getParams(2, 2));
    ...
    public function testOriginatingCourtInformationWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->originatingCourtInformation->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testOriginatingCourtInformationErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->originatingCourtInformation->get(999999999);
    ...
    public function testOriginatingCourtInformationSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->originatingCourtInformation->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testOriginatingCourtInformationResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Originating Court Information available for structure testing');
        }
    }

    public function testOriginatingCourtInformationRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->originatingCourtInformation->list(Pagination::getParams(1, 1));
                $requests++;
