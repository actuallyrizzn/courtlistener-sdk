...
/**
 * Live API tests for Parties endpoint
 * 
 * @group live
 * @group network
 * @group parties
 */
class PartiesLiveTest extends TestCase
{
    ...
    public function testListParties()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->parties->list(Pagination::getParams(1, 5));
    ...
    public function testSearchParties()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->parties->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetPartiesById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->parties->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->parties->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Parties available for testing');
        }
    }

    public function testPartiesWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->parties->list(Pagination::getParams(1, 2));
        $page2 = $this->client->parties->list(Pagination::getParams(2, 2));
    ...
    public function testPartiesWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->parties->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testPartiesErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->parties->get(999999999);
    ...
    public function testPartiesSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->parties->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testPartiesResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Parties available for structure testing');
        }
    }

    public function testPartiesRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->parties->list(Pagination::getParams(1, 1));
                $requests++;
