...
/**
 * Live API tests for Documents endpoint
 * 
 * @group live
 * @group network
 * @group documents
 */
class DocumentsLiveTest extends TestCase
{
    ...
    public function testListDocuments()
    {
        ...
        // Validate response$results = $this->client->documents->list(Pagination::getParams(1, 5));
    ...
    public function testSearchDocuments()
    {
        ...
        $searchResults = $this->client->documents->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetDocumentsById()
    {
        ...
        // Validate response$results = $this->client->documents->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->documents->get($itemId);
    ...
    public function testDocumentsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->documents->list(Pagination::getParams(1, 2));
        $page2 = $this->client->documents->list(Pagination::getParams(2, 2));
    ...
    public function testDocumentsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->documents->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testDocumentsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->documents->get(999999999);
    ...
    public function testDocumentsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->documents->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testDocumentsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Documents available for structure testing');
        }
    }

    public function testDocumentsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->documents->list(Pagination::getParams(1, 1));
                $requests++;
