...
/**
 * Live API tests for RECAP Query endpoint
 * 
 * @group live
 * @group network
 * @group recap-query
 */
class RecapQueryLiveTest extends TestCase
{
    ...
    public function testListRecapQuery()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->recapQuery->list(Pagination::getParams(1, 5));
    ...
    public function testSearchRecapQuery()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->recapQuery->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetRecapQueryById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->recapQuery->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->recapQuery->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No RECAP Query available for testing');
        }
    }

    public function testRecapQueryWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->recapQuery->list(Pagination::getParams(1, 2));
        $page2 = $this->client->recapQuery->list(Pagination::getParams(2, 2));
    ...
    public function testRecapQueryWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->recapQuery->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testRecapQueryErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->recapQuery->get(999999999);
    ...
    public function testRecapQuerySearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->recapQuery->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testRecapQueryResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No RECAP Query available for structure testing');
        }
    }

    public function testRecapQueryRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->recapQuery->list(Pagination::getParams(1, 1));
                $requests++;
