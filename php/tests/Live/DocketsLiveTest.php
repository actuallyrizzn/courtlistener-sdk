...
/**
 * Live API tests for Dockets endpoint
 * 
 * @group live
 * @group network
 * @group dockets
 */
class DocketsLiveTest extends TestCase
{
    ...
    public function testListDockets()
    {
        ...
        // Validate response$results = $this->client->dockets->list(Pagination::getParams(1, 5));
    ...
    public function testGetDocketsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->dockets->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testDocketsWithPagination()
    {
        $this->assertNotNull($this->client);
    ...
    public function testDocketsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->dockets->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testDocketsErrorHandling()
    {
        $this->assertNotNull($this->client);
    ...
    public function testDocketsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->dockets->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testDocketsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Dockets available for structure testing');
        }
    }

    public function testDocketsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
