...
/**
 * Live API tests for Non Investment Incomes endpoint
 * 
 * @group live
 * @group network
 * @group non-investment-incomes
 */
class NonInvestmentIncomesLiveTest extends TestCase
{
    ...
    public function testListNonInvestmentIncomes()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->nonInvestmentIncomes->list(Pagination::getParams(1, 5));
    ...
    public function testSearchNonInvestmentIncomes()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->nonInvestmentIncomes->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetNonInvestmentIncomesById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->nonInvestmentIncomes->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->nonInvestmentIncomes->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Non Investment Incomes available for testing');
        }
    }

    public function testNonInvestmentIncomesWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->nonInvestmentIncomes->list(Pagination::getParams(1, 2));
        $page2 = $this->client->nonInvestmentIncomes->list(Pagination::getParams(2, 2));
    ...
    public function testNonInvestmentIncomesWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->nonInvestmentIncomes->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testNonInvestmentIncomesErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->nonInvestmentIncomes->get(999999999);
    ...
    public function testNonInvestmentIncomesSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->nonInvestmentIncomes->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testNonInvestmentIncomesResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Non Investment Incomes available for structure testing');
        }
    }

    public function testNonInvestmentIncomesRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->nonInvestmentIncomes->list(Pagination::getParams(1, 1));
                $requests++;
