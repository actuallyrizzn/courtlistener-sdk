...
/**
 * Live API tests for Gifts endpoint
 * 
 * @group live
 * @group network
 * @group gifts
 */
class GiftsLiveTest extends TestCase
{
    ...
    public function testListGifts()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->gifts->list(Pagination::getParams(1, 5));
    ...
    public function testSearchGifts()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->gifts->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetGiftsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->gifts->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->gifts->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Gifts available for testing');
        }
    }

    public function testGiftsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->gifts->list(Pagination::getParams(1, 2));
        $page2 = $this->client->gifts->list(Pagination::getParams(2, 2));
    ...
    public function testGiftsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->gifts->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testGiftsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->gifts->get(999999999);
    ...
    public function testGiftsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->gifts->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testGiftsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Gifts available for structure testing');
        }
    }

    public function testGiftsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->gifts->list(Pagination::getParams(1, 1));
                $requests++;
