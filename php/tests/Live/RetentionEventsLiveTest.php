...
/**
 * Live API tests for Retention Events endpoint
 * 
 * @group live
 * @group network
 * @group retention-events
 */
class RetentionEventsLiveTest extends TestCase
{
    ...
    public function testListRetentionEvents()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->retentionEvents->list(Pagination::getParams(1, 5));
    ...
    public function testSearchRetentionEvents()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->retentionEvents->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetRetentionEventsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->retentionEvents->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->retentionEvents->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Retention Events available for testing');
        }
    }

    public function testRetentionEventsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->retentionEvents->list(Pagination::getParams(1, 2));
        $page2 = $this->client->retentionEvents->list(Pagination::getParams(2, 2));
    ...
    public function testRetentionEventsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->retentionEvents->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testRetentionEventsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->retentionEvents->get(999999999);
    ...
    public function testRetentionEventsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->retentionEvents->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testRetentionEventsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Retention Events available for structure testing');
        }
    }

    public function testRetentionEventsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->retentionEvents->list(Pagination::getParams(1, 1));
                $requests++;
