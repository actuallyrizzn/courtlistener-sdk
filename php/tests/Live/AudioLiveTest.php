...
/**
 * Live API tests for Audio endpoint
 * 
 * @group live
 * @group network
 * @group audio
 */
class AudioLiveTest extends TestCase
{
    ...
    public function testListAudio()
    {
        ...
        // Validate response$results = $this->client->audio->list(Pagination::getParams(1, 5));
    ...
    public function testSearchAudio()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->audio->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetAudioById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->audio->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->audio->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Audio available for testing');
        }
    }

    public function testAudioWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->audio->list(Pagination::getParams(1, 2));
        $page2 = $this->client->audio->list(Pagination::getParams(2, 2));
    ...
    public function testAudioWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->audio->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testAudioErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->audio->get(999999999);
    ...
    public function testAudioSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->audio->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testAudioResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Audio available for structure testing');
        }
    }

    public function testAudioRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->audio->list(Pagination::getParams(1, 1));
                $requests++;
