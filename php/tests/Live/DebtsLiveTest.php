...
/**
 * Live API tests for Debts endpoint
 * 
 * @group live
 * @group network
 * @group debts
 */
class DebtsLiveTest extends TestCase
{
    ...
    public function testListDebts()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->debts->list(Pagination::getParams(1, 5));
    ...
    public function testSearchDebts()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->debts->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetDebtsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->debts->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->debts->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Debts available for testing');
        }
    }

    public function testDebtsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->debts->list(Pagination::getParams(1, 2));
        $page2 = $this->client->debts->list(Pagination::getParams(2, 2));
    ...
    public function testDebtsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->debts->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testDebtsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->debts->get(999999999);
    ...
    public function testDebtsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->debts->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testDebtsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Debts available for structure testing');
        }
    }

    public function testDebtsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->debts->list(Pagination::getParams(1, 1));
                $requests++;
