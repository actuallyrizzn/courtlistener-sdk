...
/**
 * Live API tests for Financial endpoint
 * 
 * @group live
 * @group network
 * @group financial
 */
class FinancialLiveTest extends TestCase
{
    ...
    public function testListFinancial()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->financial->list(Pagination::getParams(1, 5));
    ...
    public function testSearchFinancial()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->financial->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetFinancialById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->financial->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->financial->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Financial available for testing');
        }
    }

    public function testFinancialWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->financial->list(Pagination::getParams(1, 2));
        $page2 = $this->client->financial->list(Pagination::getParams(2, 2));
    ...
    public function testFinancialWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->financial->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testFinancialErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->financial->get(999999999);
    ...
    public function testFinancialSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->financial->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testFinancialResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Financial available for structure testing');
        }
    }

    public function testFinancialRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->financial->list(Pagination::getParams(1, 1));
                $requests++;
