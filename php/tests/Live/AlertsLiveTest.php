...
/**
 * Live API tests for Alerts endpoint
 * 
 * @group live
 * @group network
 * @group alerts
 */
class AlertsLiveTest extends TestCase
{
    ...
    public function testListAlerts()
    {
        ...
        // Validate response$results = $this->client->alerts->list(Pagination::getParams(1, 5));
    ...
    public function testSearchAlerts()
    {
        ...
        $searchResults = $this->client->alerts->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetAlertsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->alerts->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->alerts->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Alerts available for testing');
        }
    }

    public function testAlertsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->alerts->list(Pagination::getParams(1, 2));
        $page2 = $this->client->alerts->list(Pagination::getParams(2, 2));
    ...
    public function testAlertsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->alerts->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testAlertsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->alerts->get(999999999);
    ...
    public function testAlertsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->alerts->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testAlertsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Alerts available for structure testing');
        }
    }

    public function testAlertsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->alerts->list(Pagination::getParams(1, 1));
                $requests++;
