...
/**
 * Live API tests for Disclosure Positions endpoint
 * 
 * @group live
 * @group network
 * @group disclosure-positions
 */
class DisclosurePositionsLiveTest extends TestCase
{
    ...
    public function testListDisclosurePositions()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->disclosurePositions->list(Pagination::getParams(1, 5));
    ...
    public function testSearchDisclosurePositions()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->disclosurePositions->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetDisclosurePositionsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->disclosurePositions->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->disclosurePositions->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Disclosure Positions available for testing');
        }
    }

    public function testDisclosurePositionsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->disclosurePositions->list(Pagination::getParams(1, 2));
        $page2 = $this->client->disclosurePositions->list(Pagination::getParams(2, 2));
    ...
    public function testDisclosurePositionsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->disclosurePositions->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testDisclosurePositionsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->disclosurePositions->get(999999999);
    ...
    public function testDisclosurePositionsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->disclosurePositions->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testDisclosurePositionsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Disclosure Positions available for structure testing');
        }
    }

    public function testDisclosurePositionsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->disclosurePositions->list(Pagination::getParams(1, 1));
                $requests++;
