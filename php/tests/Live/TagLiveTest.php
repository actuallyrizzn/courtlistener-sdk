...
/**
 * Live API tests for Tag endpoint
 * 
 * @group live
 * @group network
 * @group tag
 */
class TagLiveTest extends TestCase
{
    ...
    public function testListTag()
    {
        ...
        // Validate response$results = $this->client->tag->list(Pagination::getParams(1, 5));
    ...
    public function testSearchTag()
    {
        ...
        $searchResults = $this->client->tag->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetTagById()
    {
        ...
        // Validate response$results = $this->client->tag->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->tag->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Tag available for testing');
        }
    }

    public function testTagWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->tag->list(Pagination::getParams(1, 2));
        $page2 = $this->client->tag->list(Pagination::getParams(2, 2));
    ...
    public function testTagWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->tag->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testTagErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->tag->get(999999999);
    ...
    public function testTagSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->tag->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testTagResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Tag available for structure testing');
        }
    }

    public function testTagRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->tag->list(Pagination::getParams(1, 1));
                $requests++;
