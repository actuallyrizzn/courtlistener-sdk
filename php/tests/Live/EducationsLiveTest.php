...
/**
 * Live API tests for Educations endpoint
 * 
 * @group live
 * @group network
 * @group educations
 */
class EducationsLiveTest extends TestCase
{
    ...
    public function testListEducations()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->educations->list(Pagination::getParams(1, 5));
    ...
    public function testSearchEducations()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->educations->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetEducationsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->educations->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->educations->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Educations available for testing');
        }
    }

    public function testEducationsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->educations->list(Pagination::getParams(1, 2));
        $page2 = $this->client->educations->list(Pagination::getParams(2, 2));
    ...
    public function testEducationsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->educations->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testEducationsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->educations->get(999999999);
    ...
    public function testEducationsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->educations->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testEducationsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Educations available for structure testing');
        }
    }

    public function testEducationsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->educations->list(Pagination::getParams(1, 1));
                $requests++;
