...
/**
 * Live API tests for RECAP Fetch endpoint
 * 
 * @group live
 * @group network
 * @group recap-fetch
 */
class RecapFetchLiveTest extends TestCase
{
    ...
    public function testListRecapFetch()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->recapFetch->list(Pagination::getParams(1, 5));
    ...
    public function testSearchRecapFetch()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->recapFetch->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetRecapFetchById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->recapFetch->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->recapFetch->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No RECAP Fetch available for testing');
        }
    }

    public function testRecapFetchWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->recapFetch->list(Pagination::getParams(1, 2));
        $page2 = $this->client->recapFetch->list(Pagination::getParams(2, 2));
    ...
    public function testRecapFetchWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->recapFetch->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testRecapFetchErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->recapFetch->get(999999999);
    ...
    public function testRecapFetchSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->recapFetch->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testRecapFetchResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No RECAP Fetch available for structure testing');
        }
    }

    public function testRecapFetchRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->recapFetch->list(Pagination::getParams(1, 1));
                $requests++;
