...
/**
 * Live API tests for Attorneys endpoint
 * 
 * @group live
 * @group network
 * @group attorneys
 */
class AttorneysLiveTest extends TestCase
{
    ...
    public function testListAttorneys()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->attorneys->list(Pagination::getParams(1, 5));
    ...
    public function testSearchAttorneys()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->attorneys->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetAttorneysById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->attorneys->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->attorneys->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Attorneys available for testing');
        }
    }

    public function testAttorneysWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->attorneys->list(Pagination::getParams(1, 2));
        $page2 = $this->client->attorneys->list(Pagination::getParams(2, 2));
    ...
    public function testAttorneysWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->attorneys->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testAttorneysErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->attorneys->get(999999999);
    ...
    public function testAttorneysSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->attorneys->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testAttorneysResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Attorneys available for structure testing');
        }
    }

    public function testAttorneysRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->attorneys->list(Pagination::getParams(1, 1));
                $requests++;
