...
/**
 * Live API tests for Clusters endpoint
 * 
 * @group live
 * @group network
 * @group clusters
 */
class ClustersLiveTest extends TestCase
{
    ...
    public function testListClusters()
    {
        ...
        // Validate response$results = $this->client->clusters->list(Pagination::getParams(1, 5));
    ...
    public function testSearchClusters()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->clusters->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetClustersById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->clusters->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->clusters->get($itemId);
    ...
    public function testClustersWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->clusters->list(Pagination::getParams(1, 2));
        $page2 = $this->client->clusters->list(Pagination::getParams(2, 2));
    ...
    public function testClustersWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->clusters->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testClustersErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->clusters->get(999999999);
    ...
    public function testClustersSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->clusters->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testClustersResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Clusters available for structure testing');
        }
    }

    public function testClustersRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->clusters->list(Pagination::getParams(1, 1));
                $requests++;
