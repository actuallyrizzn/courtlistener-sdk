...
/**
 * Live API tests for Search endpoint
 * 
 * @group live
 * @group network
 * @group search
...
class SearchLiveTest extends TestCase
{
    ...
    public function testGetSearchById()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Search available for testing');
        }
    }
    ...
    public function testSearchErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->search->get(999999999);
    ...
    public function testSearchSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
    ...
    public function testSearchResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Search available for structure testing');
        }
    }

    public function testSearchRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->search->list(Pagination::getParams(1, 1));
                $requests++;
