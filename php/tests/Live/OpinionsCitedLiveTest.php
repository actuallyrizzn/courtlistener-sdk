...
/**
 * Live API tests for Opinions Cited endpoint
 * 
 * @group live
 * @group network
 * @group opinions-cited
 */
class OpinionsCitedLiveTest extends TestCase
{
    ...
    public function testListOpinionsCited()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->opinionsCited->list(Pagination::getParams(1, 5));
    ...
    public function testSearchOpinionsCited()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->opinionsCited->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetOpinionsCitedById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->opinionsCited->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->opinionsCited->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Opinions Cited available for testing');
        }
    }

    public function testOpinionsCitedWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->opinionsCited->list(Pagination::getParams(1, 2));
        $page2 = $this->client->opinionsCited->list(Pagination::getParams(2, 2));
    ...
    public function testOpinionsCitedWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->opinionsCited->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testOpinionsCitedErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->opinionsCited->get(999999999);
    ...
    public function testOpinionsCitedSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->opinionsCited->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testOpinionsCitedResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Opinions Cited available for structure testing');
        }
    }

    public function testOpinionsCitedRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->opinionsCited->list(Pagination::getParams(1, 1));
                $requests++;
