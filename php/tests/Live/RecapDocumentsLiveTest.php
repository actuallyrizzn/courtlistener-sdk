...
/**
 * Live API tests for RECAP Documents endpoint
 * 
 * @group live
 * @group network
 * @group recap-documents
 */
class RecapDocumentsLiveTest extends TestCase
{
    ...
    public function testListRecapDocuments()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->recapDocuments->list(Pagination::getParams(1, 5));
    ...
    public function testSearchRecapDocuments()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->recapDocuments->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetRecapDocumentsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->recapDocuments->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->recapDocuments->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No RECAP Documents available for testing');
        }
    }

    public function testRecapDocumentsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->recapDocuments->list(Pagination::getParams(1, 2));
        $page2 = $this->client->recapDocuments->list(Pagination::getParams(2, 2));
    ...
    public function testRecapDocumentsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->recapDocuments->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testRecapDocumentsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->recapDocuments->get(999999999);
    ...
    public function testRecapDocumentsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->recapDocuments->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testRecapDocumentsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No RECAP Documents available for structure testing');
        }
    }

    public function testRecapDocumentsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->recapDocuments->list(Pagination::getParams(1, 1));
                $requests++;
