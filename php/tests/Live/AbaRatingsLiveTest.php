...
/**
 * Live API tests for ABA Ratings endpoint
 * 
 * @group live
 * @group network
 * @group aba-ratings
 */
class AbaRatingsLiveTest extends TestCase
{
    ...
    public function testListAbaRatings()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->abaRatings->list(Pagination::getParams(1, 5));
    ...
    public function testSearchAbaRatings()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->abaRatings->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetAbaRatingsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->abaRatings->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->abaRatings->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No ABA Ratings available for testing');
        }
    }

    public function testAbaRatingsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->abaRatings->list(Pagination::getParams(1, 2));
        $page2 = $this->client->abaRatings->list(Pagination::getParams(2, 2));
    ...
    public function testAbaRatingsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->abaRatings->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testAbaRatingsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->abaRatings->get(999999999);
    ...
    public function testAbaRatingsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->abaRatings->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testAbaRatingsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No ABA Ratings available for structure testing');
        }
    }

    public function testAbaRatingsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->abaRatings->list(Pagination::getParams(1, 1));
                $requests++;
