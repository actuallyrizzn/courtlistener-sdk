...
/**
 * Live API tests for People endpoint
 * 
 * @group live
 * @group network
 * @group people
 */
class PeopleLiveTest extends TestCase
{
    ...
    public function testListPeople()
    {
        ...
        // Validate response$results = $this->client->people->list(Pagination::getParams(1, 5));
    ...
    public function testSearchPeople()
    {
        ...
        $searchResults = $this->client->people->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetPeopleById()
    {
        ...
        // Validate response$results = $this->client->people->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->people->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No People available for testing');
        }
    }

    public function testPeopleWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->people->list(Pagination::getParams(1, 2));
        $page2 = $this->client->people->list(Pagination::getParams(2, 2));
    ...
    public function testPeopleWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->people->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testPeopleErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->people->get(999999999);
    ...
    public function testPeopleSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->people->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testPeopleResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No People available for structure testing');
        }
    }

    public function testPeopleRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->people->list(Pagination::getParams(1, 1));
                $requests++;
