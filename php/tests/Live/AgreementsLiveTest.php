...
/**
 * Live API tests for Agreements endpoint
 * 
 * @group live
 * @group network
 * @group agreements
 */
class AgreementsLiveTest extends TestCase
{
    ...
    public function testListAgreements()
    {
        ...
        // Validate response$results = $this->client->agreements->list(Pagination::getParams(1, 5));
    ...
    public function testSearchAgreements()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->agreements->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetAgreementsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->agreements->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->agreements->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Agreements available for testing');
        }
    }

    public function testAgreementsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->agreements->list(Pagination::getParams(1, 2));
        $page2 = $this->client->agreements->list(Pagination::getParams(2, 2));
    ...
    public function testAgreementsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->agreements->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testAgreementsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->agreements->get(999999999);
    ...
    public function testAgreementsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->agreements->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testAgreementsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Agreements available for structure testing');
        }
    }

    public function testAgreementsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->agreements->list(Pagination::getParams(1, 1));
                $requests++;
