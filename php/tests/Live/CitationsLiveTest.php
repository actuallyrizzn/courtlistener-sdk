...
/**
 * Live API tests for Citations endpoint
 * 
 * @group live
 * @group network
 * @group citations
 */
class CitationsLiveTest extends TestCase
{
    ...
    public function testListCitations()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->citations->list(Pagination::getParams(1, 5));
    ...
    public function testSearchCitations()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->citations->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetCitationsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->citations->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->citations->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Citations available for testing');
        }
    }

    public function testCitationsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->citations->list(Pagination::getParams(1, 2));
        $page2 = $this->client->citations->list(Pagination::getParams(2, 2));
    ...
    public function testCitationsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->citations->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testCitationsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->citations->get(999999999);
    ...
    public function testCitationsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->citations->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testCitationsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Citations available for structure testing');
        }
    }

    public function testCitationsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->citations->list(Pagination::getParams(1, 1));
                $requests++;
