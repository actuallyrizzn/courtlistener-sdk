...
/**
 * Live API tests for Courts endpoint
 * 
 * @group live
 * @group network
 * @group courts
 */
class CourtsLiveTest extends TestCase
{
    ...
    public function testListCourts()
    {
        $this->assertNotNull($this->client);
        
        // Validate response$results = $this->client->courts->list(Pagination::getParams(1, 5));
    ...
    public function testSearchCourts()
    {
        $this->assertNotNull($this->client);
        
        $searchResults = $this->client->courts->search([
            'q' => 'test',
            'per_page' => 3
        ]);
    ...
    public function testGetCourtsById()
    {
        $this->assertNotNull($this->client);
        
        // First get a list to find a valid ID
        // Validate response$results = $this->client->courts->list(Pagination::getParams(1, 1));
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
        
        if (!empty($results['results'])) {
            $itemId = $results['results'][0]['id'];
            
            $item = $this->client->courts->get($itemId);
            
            if (empty($searchResults) || !is_array($searchResults)) { $this->markTestSkipped('API returned empty response or invalid data'); }
            $this->assertArrayHasKey('id', $item);
            $this->assertEquals($itemId, $item['id']);
        } else {
            $this->markTestSkipped('No Courts available for testing');
        }
    }

    public function testCourtsWithPagination()
    {
        $this->assertNotNull($this->client);
        
        $page1 = $this->client->courts->list(Pagination::getParams(1, 2));
        $page2 = $this->client->courts->list(Pagination::getParams(2, 2));
    ...
    public function testCourtsWithFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->courts->list($filters);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testCourtsErrorHandling()
    {
        $this->assertNotNull($this->client);
        
        // Test with invalid ID
        try {
            $this->client->courts->get(999999999);
    ...
    public function testCourtsSearchWithComplexFilters()
    {
        $this->assertNotNull($this->client);
        ...
        // Validate response$results = $this->client->courts->search($searchParams);
        if (empty($results) || !is_array($results)) { $this->markTestSkipped('API returned empty response or invalid data'); }
    ...
    public function testCourtsResponseStructure()
    {
        $this->assertNotNull($this->client);
        ...
        if (!empty($results['results'])) {
            ...
            $this->markTestSkipped('No Courts available for structure testing');
        }
    }

    public function testCourtsRateLimitHandling()
    {
        $this->assertNotNull($this->client);
        ...
        try {
            for ($i = 0; $i < $maxRequests; $i++) {
                $this->client->courts->list(Pagination::getParams(1, 1));
                $requests++;
